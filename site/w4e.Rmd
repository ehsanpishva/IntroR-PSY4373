...
There are two different exercises that you can choose from for this week. The first one is more aimed at students from FN and DN, the second for those from the PP track (but you are free to choose whatever exercise you prefer, regardless of your track).
...
6. Use the `enrichKEGG` function from the `clusterProfiler` package and use the `targets` vector you generated above as input.

7. Write the table of the KEGG pathway analysis to a file (use the `write.table` function; see the week 1 extra part) and upload that together with your script.
...
- [data_bdi.dat](https://www.dropbox.com/s/bi3s4t143n4oem3/data_bdi.dat?dl=1)
